...
function OAuthCallbackContent() {
    ...
    const processCallback = async (): Promise<void> => {
      try {
        ...
        if (callbackStatus === "success" && token) {
          ...
          const expiryTime = Date.now() + expiryMs; // Absolute timestamp
          ...
          setAccessToken(accessToken, expiryTime, "temp", ["CANDIDATE"]);
          ...
          try {
            ...
            // Now set the token with correct user info (absolute timestamp)
  ...
  return (
    <AuthLayout title="Processing OAuth Login" showSocialLogin={false}>
      <div className="login-register text-center mt-20">
        {status === "loading" && (
          <div className="mb-30">
            <div
              className="spinner-border text-brand-1"
              role="status"
              style={{ width: "3rem", height: "3rem", borderWidth: "0.3em" }}
            >
              <span className="visually-hidden">Loading...</span>
            </div>
            <h4 className="mt-30 mb-15 font-md text-brand-1">
              Completing sign-in...
            </h4>
